...
export default class LandingTemplate {
  ...
  <div class="${styles.events}" data-ng-controller="HomeController as vm">
    <div>Current User: {{ vm.currentEmail }}
      <table class="${styles["ms-Grid"]}">
        <tr>
          <th>Full Name</th>
          <th>Email</th>
          <th>Event Title</th>
        ...
        <tr data-ng-repeat="attendee in vm.attendeeCollection track by $index">
          <td>
            <input type="text" id="txtFullName" data-ng-model="attendee.FullName1">
          </td>
          <td>
            <input type="text" id="txtEmail" data-ng-model="attendee.Email">
          </td>
          <td>
      ...
      <table>
        <tr>
          ...
          <td>Email:</td>
          <td>
            <input type="text" id="txtEmail" data-ng-model="bm.newAttendeeEmail">
          </td>
        </tr>
        <tr>
          <td>
      ...
      <div>
        <table class="${styles["ms-Grid"]}">
          ...
          <tr data-ng-repeat="event in vm.eventCollection track by $index">
            <td>
              <input type="text" id="txtEventTItle" data-ng-model="event.Title">
            </td>
            <td>{{ event.StartDate | date: "MM/dd/yyyy hh:mm" }}</td>
            <td>{{ event.EndDate | date: "MM/dd/yyyy hh:mm" }}</td>
            <td>
              <input style="width: 80px;" type="text" id="txtCampus" date-ng-model="event.Campus">
            </td>
            <td>
              <input style="width: 80px;" type="text" id="txtTotal" date-ng-model="event.TotalAttendees">
            </td>
            <td>
